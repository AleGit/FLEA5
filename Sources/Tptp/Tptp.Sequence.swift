...
extension Tptp {
    struct Iterator<S, T>: Swift.IteratorProtocol {
        private var this: S?
    ...
    struct Sequence<S, T>: Swift.Sequence {
        private let this: S?
        private let step: (S) -> S?
        private let predicate: (S) -> Bool
        private let data: (S) -> T
        ...
        func makeIterator() -> Tptp.Iterator<S, T> {
            Tptp.Iterator(first: this, step: step, where: predicate, data: data)
        }
    }
}
